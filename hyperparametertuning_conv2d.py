...
def build_model(hp, N_CLASSES, SR, DT):
    ...
    n_mels = hp.Int('n_mels', min_value=40, max_value=128, step=16)
    n_fft = hp.Int('n_fft', min_value=1024, max_value=4096, step=512)
    win_length = hp.Int('win_length', min_value=256, max_value=1024, step=128)
    hop_length = hp.Int('hop_length', min_value=128, max_value=512, step=64)
...
def train(args):
    ...
    tuner = kt.RandomSearch(
        ...
        directory='hyperpara_tunining_conv2D_32k_T1_stille',
    ...
    tuner.search(tg, validation_data=vg, epochs=15, callbacks=[tensorboard_callback])
