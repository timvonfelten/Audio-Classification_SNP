...
def build_model(hp, N_CLASSES, SR, DT):
    ...
    n_mels = hp.Int('n_mels', min_value=16, max_value=128, step=8)
    n_fft = hp.Int('n_fft', min_value=1024, max_value=4096, step=512)
    win_length = hp.Int('win_length', min_value=512, max_value=2048, step=256)
    hop_length = hp.Int('hop_length', min_value=256, max_value=2048, step=128)
    dropout_rate = hp.Float('dropout_rate', min_value=0.1, max_value=0.6, step=0.05)
    activity_regularizer = hp.Float('activity_regularizer', min_value=0.0001, max_value=0.01, step=0.001)
...
def train(args):
    ...
    tuner = kt.RandomSearch(
        ...
        directory='hyperpara_tunining_lstm_T3',
    ...
    tuner.search(tg, validation_data=vg, epochs=16, callbacks=[tensorboard_callback])
